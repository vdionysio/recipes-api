...
describe('PUT /recipes/:id/image', () => {
  describe('quando a requisição é realizada com sucesso', () => {
    ...
    before(async () => {
      ...
      response = await chai.request(server)
        ...
        .set('authorization', token)
        .attach('image', fs.readFileSync(path.join(__dirname, 'test-image.jpg')), `${recipe['_id']}.jpg`);
      });
    ...
    it('com uma prop image onde o id da receita está incluso no seu nome', () => {
      const recipe = response.body;
      expect(recipe).have.a.property('image');
      expect(recipe.image.includes(recipe['_id'])).to.be.true;
    });
  });
  ...
  describe('quando o id passado é inválido', () => {
    ...
    before(async () => {
      ...
      // add image by id
      response = await chai.request(server)
        .put(`/recipes/${fakeId}/image`)
        .set('authorization', token)
        .attach('image', fs.readFileSync(path.join(__dirname, 'test-image.jpg')), `${fakeId}.jpg`);
    });
  ...
  describe('quando a receita não pertence ao usuário autenticado', () => {
    ...
    before(async () => {
      ...
      // edit recipe by id
      response = await chai.request(server)
        .put(`/recipes/${recipe1['_id']}/image`)
        .set('authorization', token2)
        .attach('image', fs.readFileSync(path.join(__dirname, 'test-image.jpg')), `${recipe1['_id']}.jpg`);
    });
